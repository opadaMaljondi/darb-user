...
class HomeRepositoryImpl implements HomeRepository {
  ...
  //     if(mapType == 'google_map'){
  //       if(response.data.toString() != '{}' && response.data['results'] != []){
  //         for (var i = 0; i < response.data['results'].length; i++) {
  //           if(response.data['results'][i]['address_components'][0]['types'][0] != 'plus_code'){
  ...
  //       }
  //     } else{
  //       printWrapped('text --------- '+response.toString());
  //       final displayName = response.data['display_name'];
  //       placesResponse = displayName?.toString() ?? '';
